...
#
##############
#errorreturned +=  str(input_data['takeInfoFromShopify']) + ' *** '
if input_data['takeInfoFromShopify'] == True or input_data['takeInfoFromShopify'] == 'True': 
    #errorreturned +=  str(errorQueryShopify) + ' *** '
    if errorQueryShopify == False:
        ...
        #errorreturned +=  str(ordersToday) + ' *** '
        # for item in dict.keys(ordersToday):       
        #     errorreturned +=  str(item) + ' *** ' 
        #     if str(item) != 'date':
        #         errorreturned +=  str(item) + ' *** '                
        #         # order = ordersToday['orders'][item]
        #         arr_todays_sales.append (float(ordersToday[item]['orders']['total_price']))
        #         errorreturned +=  str(ordersToday[item]['orders']['total_price']) + ' *** '
        #         todays_sales += float(order[item]['orders']['total_price'])
        #         total_orders += 1
        #         online_orders += 1
        #         online_sales += float(order[item]['orders']['total_price'])
        #         # Filter orders with a min value
        #         if float(order[item]['orders']['total_price']) > float(input_data['min_value']):
        #             new_customers, first_time_reorders, converted_by_samples, wonback_customers = categorizeSale (order[item]['orders'], new_customers, first_time_reorders, converted_by_samples, wonback_customers)
        #         for product in range(len(order[item]['orders']['line_items'])):
        #             product_name = str(order[item]['orders']['line_items'][product]['name']) 
        #             if product_name.find('Varnish') != -1:
        #                 units_of_varnish_sold += int(order[item]['orders']['line_items'][product]['quantity']) 
        #             elif product_name.find('Prophy') != -1:    
        #                 units_of_prophy_sold += int(order[item]['orders']['line_items'][product]['quantity'])

        for item in dict.keys(ordersToday):            
            if str(item) != 'date':
                order = ordersToday[item]
                #print(str(order['line_items'][0]['name']))
                ...
                if float(order['total_price']) > float(input_data['min_value']):
                    new_customers, first_time_reorders, converted_by_samples, wonback_customers = categorizeSale (order, new_customers, first_time_reorders, converted_by_samples, wonback_customers)
